...
- [ ] **Phase 6: TUI Cockpit** (In progress — 10/11 plans: 6 original complete 2026-08-28 + 4/5 gap-closure from 06-UAT) - Ratatui cockpit exposing every CLI action: dashboard, log tail, tag browser with live watch, alarm panel, project browser, rig screen; CLI<->TUI coverage CI-enforced (tui_coverage clap-tree walk); gap closure: tags freshness, TTY hint, f64 gauges, root-level puts, modal geometry/vim motions/prose menus (06-10); remaining 06-11: rig status render + README keymap sync
