...
| 6    | target_state  | command invalid for the gateway's current state    | `gateway_too_old`, `gateway_not_commissioned`, `gateway_restarting`, `not_found`, `project_exists`, `resource_binary`, `trial_not_expired`, `provider_not_found`, `routes_not_deployed`, `webdev_unlicensed`, `route_version_mismatch`, `webdev_route_error`, `tag_collision`, `alarm_journal_missing`, `import_denied`, `session_not_prunable`, `eam_not_controller`, `eam_task_type_refused`, `eam_task_in_flight`, `script_exec_not_configured`, `lint_tool_absent`, `provider_root_unsupported` |
...
| `ign diagnostics bundle generate` | Start a diagnostics support-bundle generation (POST, no body) — the 200 answer IS the fresh status | capture-encoded state machine (8.3.3 + 8.3.6, 09-02 rigs): the observed vocabulary is EXACTLY `Generating` (mid-generation) and `Valid` (terminal/ready) — PascalCase; `state` is a String, never an enum, so an unobserved future state rides verbatim and never refuses; generation takes ~2–6 s on a fresh rig — pair with `wait`; JSON data `{state}`; authed read |
...
| `ign diagnostics bundle wait [--interval SECS] [--timeout SECS]` | Poll the status until a terminal captured state (`Valid`) — defaults 2 s / 300 s (the restart --wait shape) | **the captured vocabulary drives termination**: any captured non-generating state is terminal; while `Generating` it keeps polling; an UNKNOWN state (outside the captured vocabulary) is honestly NOT terminal — it keeps polling until the deadline and the final observed status rides the deadline message; deadline expiry is exit 4 `network_error` (the restart-wait convention — NO new slug); JSON data `{state, fileSize}`; authed read |
